...
$(ONT)-base.owl: $(EDIT_PREPROCESSED) $(OTHER_SRC) $(IMPORT_FILES) $(OWL_INFERRED_HIERARCHY_FILES)
	...
	remove --base-iri $(URIBASE)/WMBO --base-iri $(URIBASE)/PCL --base-iri $(URIBASE)/pcl/CS20250428 --base-iri $(BICANBASE)/CCN20250428 --base-iri $(URIBASE)/CL_43 --base-iri $(URIBASE)/CLM_5 --axioms external --preserve-structure false --trim false \
...
$(ONT)-simple.owl: $(EDIT_PREPROCESSED) $(OTHER_SRC) $(SIMPLESEED) $(IMPORT_FILES) $(OWL_INFERRED_HIERARCHY_FILES)
	$(ROBOT_RELEASE_IMPORT_MODE) \
		...
		relax \
		filter --term-file $(SIMPLESEED) --select "annotations ontology anonymous self <http://purl.obolibrary.org/obo/PCL_*>" --select "<https://purl.brain-bican.org/taxonomy/*>" --select "<http://purl.obolibrary.org/obo/CL_43*>" --select "<http://purl.obolibrary.org/obo/CLM_5*>" --trim true --signature true \
		reduce -r ELK --preserve-annotated-axioms true \
